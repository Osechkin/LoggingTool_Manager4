...
{		
	...
	gf_data->index = 1;			// номер порождающего полинома, который равен максимальному количеству исправл€емых ошибок плюс 1  
	gf_data->index_hdr = 1;
	...
	// расположение окна приложени€ по центру экрана. Ќе работает под Linux
...
{
	// поле field_gradient не отправл€етс€ в DSP, т.к. совсем там не нужно.
...
{	
	...
	emit sdsp_is_enabled(false);	// удалить, т.к. имеетс€ в строке 2224 !
...
void MainWindow::calcT2Spectra(DataSets &forT2_spec_dss)
{
	...
	for (int i = 0; i < forT2_spec_dss.count(); i++)
	{
		...
		QVector<double> *T2 = new QVector<double>(spec_size);			// ¬ремена “2, по которым строитс€ спектр
		...
		{		
			T2->data()[j] = T2min * exp(j*dT2);							// Ќовый вариант		
